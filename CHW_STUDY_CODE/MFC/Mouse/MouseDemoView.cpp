...
void CMouseDemoView::OnPaint()	//뷰는 글씨를 추가하기위해서 OnPaint 에서 해야한다.
{
...
void CMouseDemoView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	//m_nX = point.x;
	//m_nY = point.y;
	//RedrawWindow(); //invalidateRect 처럼 WM_PAINT 발생.. OnPaint() 을바로 부르면 안된다. 왜냐면 On이 있는 함수는 콜백으로만 불려야하기때문에
