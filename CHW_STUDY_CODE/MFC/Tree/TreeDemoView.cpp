...
void CTreeDemoView::OnInitialUpdate()
{
	...
	HTREEITEM hHal;
	hHal = m_ctrlFamily.InsertItem(_T("할아버지"), 0, 1, TVI_ROOT);//최상위 이다.	//첫번쨰 0은 일반상태, 1은 눌렀을떄 상태의 이미지 이다.

	HTREEITEM hBigdad;
	hBigdad = m_ctrlFamily.InsertItem(_T("큰아버지"),2 , 0, hHal);	//부모가 hHal이다.
	m_ctrlFamily.InsertItem(_T("사촌형"), 0, 0, hBigdad);
	m_ctrlFamily.InsertItem(_T("사촌누나"), 0, 0, hBigdad);

	HTREEITEM hDad, hMe;
	hDad = m_ctrlFamily.InsertItem(_T("아버지"), 2, 0, hHal);
	m_ctrlFamily.InsertItem(_T("형"), 0, 0, hDad);
	hMe = m_ctrlFamily.InsertItem(_T("나"), 0, 0, hDad);
	m_ctrlFamily.InsertItem(_T("동생"), 0, 0, hDad);
	
	HTREEITEM hSmallDad;
	hSmallDad= m_ctrlFamily.InsertItem(_T("작은아버지"), 2, 0, hHal);
	m_ctrlFamily.InsertItem(_T("사촌동생"), 0, 0, hSmallDad);

	m_ctrlFamily.EnsureVisible(hBigdad);//처음에 킬때 펼쳐져서 보이는것 보장
