    ...
    case WM_PAINT:
        {
            ...
            TextOut(hdc, 100, 100, _T("Hello world"),5);
            TextOut(hdc, 100, 200, _T("최현우"), 3);

            TextOut(hdc, 100, 300, _T("Do you love me?"), 20);

            MoveToEx(hdc, 50, 50, NULL); //(마지막인자)lpPoint:이전 현재 좌표를 돌려받기 위한 POINT구조체의 포인터. 이전 좌표가 필요없으면 NULL을 전달해 준다.
            LineTo(hdc, 300, 90);

            RoundRect(hdc, 500, 200, 700, 300, 10, 10);
            Rectangle(hdc, 50, 100, 200, 180);
            Ellipse(hdc, 50, 100, 200, 180);

            //TextOut(hdc,10,10, _T("안녕"),2); //10,10 좌표에 2크기 만큼 출력하라. TextOutA(ansi코드)에서는 한글 한글자만 나오고, TextOutW(유니코드)는 두글자가 나온다.
                                              // 그래서 크기를 상수로 주는게 아니고 범용타입 _tstrlen ?? 을사용해서 크기를 주는게 좋다.
            EndPaint(hWnd, &ps);
        }
        break;
    case WM_DESTROY:
