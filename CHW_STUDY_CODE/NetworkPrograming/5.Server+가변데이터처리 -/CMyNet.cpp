...
void CMyNet::Run()
{
	...
	printf("클라이언트 연결 대기:192.168.0.86:9000\n");
...
{
	//SendData  처럼 처음에 4바이트는 가변데이터의 크기를 보내주고 두번쨰로 데이터를 넘겨주기 떄문에 Recvn을 두번한다.

	//1. 4byte 헤더를 획득 , Recvn에 한번들어가서 size에 가변데이터(받을데이터?)의 크기를 받아온다.
	int size;
	int retval = g_mynet->Recvn(s, (char*)&size, sizeof(int), 0);
	
	//2. 가변 데이터 획득. 위애서 받아온 size크기만큼 데이터를 buf에 받는다?
...
int CMyNet::SendAllData(SOCKET sock, const char* msg, int size)
{
	//전체 전송(1대 다 통신)
	int retval = 0;
	for (int i = 0; i < (int)g_mynet->clients.size(); i++) //<====
