...
#define DATA_FILE_NAME "C:\\Temp\\AddrBook_List.dat"
...
void Add()
{
    ...
    if (nChoice == 1) {
        ...
        strcpy_s(pNewUser->strName, sizeof(pNewUser->strName), strName);
        strcpy_s(pNewUser->strPhoneNumber, sizeof(pNewUser->strPhoneNumber), strPhoneNum);
        pNewUser->nAge = nAge;
    ...
    if (nChoice == 1) {
        ...
        g_AddrBook[g_nCount].pNext = g_AddrBook + (g_nCount + 1);
...
void PrintAll()
{
    ...
    while (pTemp->pNext != NULL)
    {
       printf("%s, %d, %s, %s\n",
            pTemp->strName,
            pTemp->nAge,
            pTemp->nGender == 0 ? "남성" : "여성",
            pTemp->strPhoneNumber);
...
void Serch()
{
    ...
    while (pTemp != NULL)
    {
        cmpResult = strcmp(pTemp->strName, strName);
        if (cmpResult == 0)
        {
            printf("이름 : %s\t ,성별 : %s\t, 나이 : %d세\t,  전화번호 : %s\n",
...
void Delete()
{
    ...
    while (1)
    {
        ...
        if (cmpResult == 0)
        {
            pPev->pNext = pTemp->pNext;
            //pTemp = NULL;

            //if (pTemp == g_Head)//1st Node will be deleted.
             //   g_Head = pTemp->pNext;
...
int NameSerch(char* name)
{
    ...
    while (pTemp != NULL)
    {
        cmpResult = strcmp(pTemp->strName, name);
        if (cmpResult == 0)
            return 1;
        pTemp = pTemp->pNext;
    }
