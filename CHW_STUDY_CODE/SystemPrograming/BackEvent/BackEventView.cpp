...
CBackEventView::CBackEventView() noexcept
{
	...
	hEvent30 = CreateEvent(NULL, FALSE, FALSE, NULL);// 2번째 인자가 false면 자동 리셋 이벤트(자동청소)
	hEventCalc = CreateEvent(NULL, TRUE, FALSE, NULL); // 한개의 이벤트를 여러곳에서 받아야한다면 자동으로 주지말고 수동 ture로 해야한다.
														//근데 한번 메시지를 받고 청소를 안해주면 전에 편지를 그대로 사용한다.  
}
...
void CBackEventView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	WaitForSingleObject(hEvent30, INFINITE); //hEvent30이벤트가 발동시 발동.
	////////////////

	//첫화면 먼저 그려주는 일을 할때 이런식으로 함.
	CDC* pDC = GetDC();
	CString str;
	for (int i = 0; i <= 30; i++)
	{
