...
CSemDownView::CSemDownView() noexcept
{
	...
	hSemDown = CreateSemaphore(NULL, 3, 3, NULL);	//3개만 사용하는 세마포어 생성
}
...
DWORD WINAPI ThreadDownload(LPVOID p)
{
	...
	srand(GetTickCount());
	int s = rand() % 5 + 1; //다운로드 속도를 실제처럼 보이려고 난수 사용

	///////// 컨텍스 오버헤드 발생가능.
	Y += 20;
	int  y = Y;
	//////////// 여기를 크리티컬 섹션같은거로 넣던지 하는게 좋음.
	...
	for (int i = 0; i < 100; i++)
	{
		strTemp.Format(_T("다운로드 중 : %d%% 완료"), i);
		for (int j = 0; j < i; j++)
			strTemp += _T("|");
	...
	//////////////////////////////////////////////////////////
	ReleaseSemaphore(hSemDown, 1, NULL); //1개의 세마포어 해제

	pDC->TextOut(10, y, _T("다운로드 완료"));
	pView->ReleaseDC(pDC);
	return 0;
}
